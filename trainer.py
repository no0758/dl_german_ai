...
class Trainer(object):
    ...
    def __init__(self, model=None, criterion=None, optimizer=None, dataset=None,
        ...
        self.step_size = 2
        self.scheduler = StepLR(optimizer, step_size=self.step_size, gamma=0.5)
    ...
    def test(self):
        ...
        for i, data in enumerate(self.valid_dataset):
            ...
            loss = self.get_loss(batch_output, target_var)
            e_loss += loss.item() * input_var.shape[0]  #####
            pred = torch.argmax(batch_output.data, 1)
            accu_num += torch.sum(pred == batch_target.data)  #####
            percent = float(i) * 100 / float(batch_num)
            sys.stdout.write("%.4f" % percent);
            sys.stdout.write("%\r")
            sys.stdout.flush()
    ...
    def test_and_save(self,mode='onehot'):
        # Set the model to be in testing mode (for dropout and batchnorm)
        self.model.eval()  #####
        e_loss = 0
        accu_num = 0
        ...
        for i, data in enumerate(self.valid_dataset):
            ...
            pred = torch.argmax(batch_output.data, 1)
            accu_num += torch.sum(pred == batch_target.data)#####
            preds.extend(pred.cpu().numpy() if torch.cuda.is_available() else pred.numpy())
            percent = float(i) * 100 / float(batch_num)
            sys.stdout.write("%.4f" % percent);
            sys.stdout.write("%\r")
            sys.stdout.flush()
